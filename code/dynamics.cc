...
// let the state evolves until the next wall hit or particle collision
	void Dynamics::next_step(double time_to_next_frame)
		{
		...
		int i;
		if(time_step > time_to_next_frame) 
			{
			for(i=1; i <= Np(); i++)
				particles[i-1].Update(time_to_next_frame);
			time+=time_to_next_frame;
			return;
			}
		for(i=1; i <= Np(); i++)
...
// let the state evolves between samplings
	void Dynamics::go_to_next_sampling(int make_movie, double movie_time_step)
		...
		if(make_movie) number_of_steps*=10;
		init_counters();
		int frame = 1;
		for(int i=1; number_of_wall_hits+number_of_collisions <= number_of_steps; i++)
			{
			next_step(frame*movie_time_step-time);
			if(make_movie && time >= frame*movie_time_step) 
				{
				cout << *this << endl;
				frame++;
				}
